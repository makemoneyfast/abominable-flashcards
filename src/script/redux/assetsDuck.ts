...
export default function reducer(
  ...
  switch (action.type) {
      ...
      return {
        ...
        kanji: {
          ...
          [action.payload.kanji]: {
            ...
            kunyomiAccent: action.payload.kunyomiAccent,
            onyomi: action.payload.onyomi,
            onyomiAccent: action.payload.onyomiAccent,
    ...
    case SAVE_EXISTING_CARD:
      ...
      return {
        ...
        kanji: {
          ...state.kanji,
          [action.payload.id]: {
            character: action.payload.buffer.kanji,
            meaning: action.payload.buffer.meaning,
            notes: action.payload.buffer.hint,
            tags: action.payload.buffer.tags.slice().sort(),
    ...
    case DELETE_SET:
      ...
      setCollection = _(state.allSets).without(action.payload).value();
      return {
        ...state,
        sets: allSets,
    ...
    case DELETE_CARD:
      ...
      for (let key in allSets) {
        ...
        if (set.kanji.indexOf(action.payload) >= 0) {
          ...
          allSets[key] = {
            ...set,
            kanji: _(set.kanji).without(action.payload).value(),
          };
        }
      }
      return {
            ...
            for (let tag of action.payload.tagOperation) {
              ...
              if (tagIndex >= 0) {
                updatedKanji.tags = updatedKanji.tags
                  .slice(0, tagIndex)
                  .concat(updatedKanji.tags.slice(tagIndex + 1));
              }
            }
