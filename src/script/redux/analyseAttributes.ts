  ...
  for (const flagSet of flagSets) {
    for (let flagName in flagSet.mask1) {
      if (flagSet.mask1.hasOwnProperty(flagName)) {
        if ((mask1 & flagSet.mask1[flagName]) === flagSet.mask1[flagName]) {
    ...
    for (let flagName in flagSet.mask2) {
      if (flagSet.mask2.hasOwnProperty(flagName)) {
        if ((mask2 & flagSet.mask2[flagName]) === flagSet.mask2[flagName]) {
          flagNames.push(flagName);
        }
      }
    }
    ...
    .map((bitMask, flagName) => {
      ...
      if (attributesWithFlagSet.length > 0) {
        return {
          ...
          percentage:
            Math.floor(
              (attributesWithFlagSet.length / population.length) * 1000
            ) / 10,
...
function makeSummary(key: string, items: Attribute[], total: number) {
  const flagFingerprintReports = auditFingerprints(items, makeFlagFingerprint);
...
export function analyseAttributes(rawData: string) {
  ...
  const byDestination = _(allAttributes)
    .filter(
      (attribute) =>
        attribute.categoryNumber == "2" || attribute.categoryNumber == "5"
    )
    ...
    .map((attributes) => {
      ...
      const supportCost = _(uniqueFingerprints).reduce((result, value, key) => {
        const costTable = {
          string: 0,
          "": 1,
          dropDown: 10,
  ...
  return {
    ...
    constraints: auditIndividualFlags(filteredAttributes, ConstraintFlagSet),
    controls: _(filteredAttributes)
      ...
      .map(
        ...
        ) => makeSummary(typeFingerprint, attributes, filteredAttributes.length)
      )
      .value()
      .sort(sortByPercentage),
  };
